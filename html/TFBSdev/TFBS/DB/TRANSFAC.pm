...
=head2 new
 ...
 Title   : new
 Usage   : my $db = TFBS::DB::TRANSFAC->new(%args);
 Function: the formal constructor for the TFBS::DB::JASPAR2 object;
	   here same as I<connect>
 Returns : a TFBS::DB::TRANSFAC object
 Args    : -accept_conditions # OPTIONAL: by setting this to true 
...
=head2 connect
 ...
 Function: Here, I<connect> is just a synonim for I<new>
...
sub _get_Matrix_by_URL  {
    ...
    foreach my $line (@lines)  {
        ...
        elsif ($line =~ /^[012][0-9]:\s+/)  {
            my ($nr, $A, $C, $G, $T) = split /\s+/, $line;
