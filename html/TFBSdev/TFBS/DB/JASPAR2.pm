...
Albin Sandelin and Wyeth W. Wasserman. Besides theprofile matrix itself,
...
=head2 new
 ...
 Title   : new
 Usage   : my $db = TFBS::DB::JASPAR2->new(%args);
 Function: the formal constructor for the TFBS::DB::JASPAR2 object;
 ...
 Args    : # you must specify either one of the following two:
 
	   -connect, # reference to an array containing
		     # three elements:
		     # dbi connectstring, database username
		     # and password, e.g.:
	      ...
	      #or
	   -create,  # same as -connect, but the connectstring
		     # contains the name of a non-existant database,
		     # which is then created:
		     # -create  => ['dbi:mysql:NEWJASPAR2:myhost',
				    'myusername',
				    'mypassword'
				   ]
