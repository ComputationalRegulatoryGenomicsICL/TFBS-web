...
=header2 Iterator
  ...
  Title   : Iterator
  Usage   : my $it = $sitepairset->Iterator()
            my $it = $sitepairset->Iterator(-sort_by=>'start', -reverse=>1)
...
sub _get_set  {
	...
	$siteset->add_site($site_pair->$feature);
    }
    return $siteset;
}
